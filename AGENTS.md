...
- Treat the local TRX catalog as the source of truth for exercise metadata. Keep nullable `force`, `mechanic`, `category`, and `variationGroup` fields on `exercises`, populate all catalog entries through `lib/db/seed-exercises.ts`, and do not import free-exercise-db records into PostgreSQL.
...
- Pass only closed-catalog planning data to the workout LLM: exercise ID, name, primary pattern, force, mechanic, category, difficulty, unilateral flag, muscles, and optionally variation group. Do not include image URLs, source URLs, or timestamps, and continue validating that generated exercise IDs belong to the supplied candidate pool.
- Keep the deterministic workout fallback on the same enriched candidate selector used by LLM generation so difficulty, recency, force, mechanic, category, pattern, and variation diversity rules remain active when OpenRouter is unavailable.
