...
- Treat the local mixed TRX and dumbbell catalog as the source of truth for exercise metadata. Keep non-null `equipment` and nullable `force`, `mechanic`, `category`, and `variationGroup` fields on `exercises`, and populate all catalog entries through `lib/db/seed-exercises.ts`. Source dumbbell exercises from free-exercise-db and include only movements requiring dumbbells and the floor, without a bench, chair, box, rack, ball, platform, or other accessory; do not bulk-import unrelated free-exercise-db records.
- Keep persisted `variationGroup` as the authoritative close-substitute grouping for workout selection; do not infer it from `primaryPattern`, broad families, or slug heuristics. Prefer distinct groups while alternatives exist and relax only to keep the candidate pool viable.
- Keep workout candidate selection in `lib/workouts/workout-candidate-selector.ts` probabilistic. Preserve difficulty filtering, recent-usage penalties, movement-pattern coverage, balanced TRX/dumbbell representation, muscle-similarity penalties, and weighted random sampling; use `mechanic`, `category`, and `force` as moderate goal/focus/diversity signals rather than hard filters.
- Pass only closed-catalog planning data to the workout LLM: exercise ID, name, primary pattern, force, mechanic, category, equipment, difficulty, unilateral flag, muscles, and optionally variation group. Do not include image URLs, source URLs, or timestamps, and continue validating that generated exercise IDs belong to the supplied candidate pool. Tell the LLM that dumbbell movements may use only dumbbells and the floor.
- Keep a single two-request retry budget for each AI-assisted workout generation. On HTTP 429, respect `Retry-After` when present, use a bounded local cooldown, and use the deterministic fallback immediately instead of retrying the rate-limited request.
- Keep the deterministic workout fallback on the same enriched candidate selector used by LLM generation so difficulty, recency, balanced equipment representation, force, mechanic, category, pattern, and variation diversity rules remain active when Mistral is unavailable.
- Keep verified exercise image fixtures in `lib/db/seed-exercise-images.ts` as external asset URLs from public official TRX Training pages or high-confidence free-exercise-db mappings; do not download assets, copy them into `public/`, or scrape sources at application runtime.
- Model exercise image mappings explicitly with their target slug, source, source exercise/page, image URL, and confidence. Insert only high-confidence mappings automatically; report medium-confidence candidates without inserting them, and prefer a correct placeholder over a misleading pose or materially different equipment setup.
- Resolve exercise images by slug, treat `(exerciseId, url)` as the idempotency key in seed logic, and never remove images added outside the seed.
- Validate configured image URLs for a successful image response before adding them to the seed. Convert free-exercise-db relative image paths to stable `raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/` URLs, and never persist temporary clone paths.
- Run `npm run db:seed:exercise-images` to add verified images for exercises currently present in the database; report total coverage, missing slugs, rejected medium-confidence mappings, and configured slugs that are absent. Run the seed twice after mapping changes to confirm idempotency.
